...
void solve(int argc,char* argv[]){
	...
	double prev = start - (n-1);
	for(int i = start - (n-1);i<start;i++){
		sum += price[i];
		sum_of_squares += price[i]*price[i];
	}
	ll position = 0;
	double cash = 0;
	...
	for(start;start<len(dates);start++){
		sum += price[start];
		sum_of_squares += price[start]*price[start];
		double DMA = (double)(sum/(double)n);
		double SD = sqrt((double)(sum_of_squares/(double)n) - (double)DMA*DMA);
		if(price[start] - DMA >= p*SD){
			if(position < x){
				position++;
				cash -= price[start];
		...
		sum -= price[prev];
		sum_of_squares -= price[prev]*price[prev];
		prev++;
